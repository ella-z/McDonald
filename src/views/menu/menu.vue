<template>
  <div class="container">
    ...
    <cube-slide ref="slide" :data="items" class="slide">
      <cube-slide-item
        ...
        <div class="Card" ref="righ">
          <div>
            <div ref="good" :key="index" v-for="(item,index) in TabList">
              <ul class="rcontent">
                <li
                  v-for="(food,submenuIndex) in item.submenu"
                  :key="submenuIndex"
                  :class="{big:index<2}"
                ...
                >
                  ...
                  <div class="single" v-show="index>2">
                    <i
                      class="iconfont"
                      v-show="item.submenu[submenuIndex].count"
                      @click.stop="reduceCar(index,submenuIndex)"
                    >&#xe620;</i>
                    <span
                      class="itemsCount"
                      v-show="item.submenu[submenuIndex].count"
                    >{{item.submenu[submenuIndex].count}}</span>
                    <i class="iconfont" @click.stop="addCar(index,submenuIndex)">&#xe626;</i>
                  </div>
                </li>
              </ul>
...
export default {
  ...
  mounted() {
    var that = this;
    this.axios
      .post("http://localhost:80/mcdonald/poster.php")
  ...
  methods: {
    ...
    toMenuCard(index, submenuIndex) {
      ...
      this.$router.push({
        path: "/MenuCard",
        query: { indexID: index, submenuIndex: submenuIndex }
      });
    }
  }
};
